...
export const phaseLabel = (a,config) => a.archived ? '已封存' : !a.accepting ? (migrationReadOnly(a,config)?'搬遷資料・僅供查閱':'尚未開放交件') : ({production:'製作與上傳',review:'初讀與改留',exhibit:'班內展示'}[a.phase] || '查閱作品');
export const supported = a => ['w4','w5-workshop'].includes(a.kind);
...
export function studentNext(b,config) {
  ...
  if(!supported(a))return ['查閱以前的作品','這個活動保留原系統紀錄；新增操作請回原作品牆。'];
  ...
  if(b.reviews.some(r=>r.status==='assigned')&&a.phase==='review')return ['有一份初讀等你完成','切換「我的初讀任務」，先看圖，再寫出你的理解與根據。'];
  if(!w?.versions.length)return [a.kind==='w4'?'先保存我的圖卡':'先保存小組的兩張 A3',a.kind==='w4'?'原意留在歷程本，這裡只上傳圖卡。':'同組交一份，組員先確認加入，再上傳 A、B 題照片。'];
  if(w.feedback.length&&!w.decisions.length&&a.kind==='w4'&&a.phase==='review')return ['同學的初讀回來了','比對原意與畫面，再決定修改，或有理由地保留。'];
