...
async function setup(){
  ...
  try{
    ...
    banner.innerHTML=`<div><strong>學生視角 · 唯讀</strong><p>${esc(c.className)} 班 ${esc(student.seat)} 號 · ${esc(student.name)}</p><small>顯示這位學生目前看得到的內容；不能代替他保存、送出或勾選同意。</small></div><div class="preview-controls"><label for="previewSwitch">切換學生</label><select id="previewSwitch">${c.students.map((s,i)=>`<option value="${i}" ${i===index?'selected':''}>${esc(s.seat)} 號 · ${esc(s.name)}</option>`).join('')}</select><div>${index>0?`<a href="${esc(target(index-1))}">← 上一位</a>`:''}${index<c.students.length-1?`<a href="${esc(target(index+1))}">下一位 →</a>`:''}<a class="preview-exit" href="${esc(teacherUrl+'&className='+encodeURIComponent(c.className))}">返回教師畫面</a></div></div>`;
