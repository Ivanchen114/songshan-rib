...
async function start(){try{
 ...
 else{$('#status').textContent=r?(r.status==='submitted'?'已送出反思；可繼續修改。':'已保存草稿，可接著寫。'):'欣賞 → 引用 → 寫自己的 ORID。';}
 if(!teacher&&r&&params.get('reference')&&params.get('reference')!==r.referenceVersionId){dirty=true;$('#status').textContent='已選另一件引用作品，請核對三問後保存。';}
...
async function save(e){e.preventDefault();if(busy)return;const form=e.target,values=new FormData(form),answers=Object.fromEntries(['o','r','id'].map(k=>[k,values.get(k)])),mode=e.submitter.value;
 ...
 if(!teacher)model.reflection={answers,status:mode,allowPublic:values.has('allowPublic'),revision:result.revision};dirty=false;requestId=crypto.randomUUID();$('#status').textContent=teacher?'匿名展示副本已保存；作品公開時才會一起顯示。':mode==='draft'?'草稿已保存，稍後可回自己的作品繼續。':'反思已送出，已留在自己的作品旁。';$('#status').className='saved';
