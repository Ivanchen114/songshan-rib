...
export function createAiJudgmentUI(ctx){
 ...
 async function action(el){
  ...
  const context=`<section aria-label="對照圖卡與甲乙留言"><h3>我的 W4 V${source.ordinal} 圖卡</h3>${source.taskNote?`<p class="notice">${esc(source.taskNote)}</p>`:''}<div class="ai-reading-image"><img src="${esc(source.imageUrl)}" alt="這次判讀對照的我的 W4 圖卡"></div>${source.commentA?`<div class="ai-reading-comments"><section><h3>甲</h3><p>${esc(source.commentA)}</p></section><section><h3>乙</h3><p>${esc(source.commentB)}</p></section></div>`:''}</section>`;
