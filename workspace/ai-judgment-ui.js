...
export function createAiJudgmentUI(ctx){
 ...
 async function action(el){
  if(el.dataset.action==='demoPlan'){
   ...
   state.demoPlan=plan;
   show(`<h2>發放示範練習</h2><p>${esc(plan.term)} 學期 · ${esc(plan.className)} 班</p><p>只列仍未上傳 W4 圖片且尚未領取示範的學生。示範不列入個人交件；已交但圖片空白或看不清楚者，保留補件提醒，不自動列入。</p><ul>${plan.candidates.map(s=>`<li>${s.seat} 號 ${esc(s.name)} → 示範 ${s.code}｜${esc(s.title)}</li>`).join('')}</ul><p>待發 ${plan.candidates.length} 人；已發且仍未交 ${plan.alreadyAssigned} 人。下次可重複執行，會重新查名單並跳過已發者。</p>${plan.candidates.length?'<button data-action="assignDemos">確認發給以上同學</button>':'<p>目前沒有需要新增發放的學生。</p>'}`);return true;
  }
  if(el.dataset.action==='assignDemos'){
   ...
   state.demoPlan=null;dialog.close();await board();say(`已發放 ${result.assigned} 份示範；學生 W4 交件狀態未改。`);return true;
  }
  ...
  const context=`<section aria-label="對照圖卡與甲乙留言"><h3>${source.isDemo?esc(source.label):`我的 W4 V${source.ordinal} 圖卡`}</h3>${source.taskNote?`<p class="notice">${esc(source.taskNote)}</p>`:''}<div class="ai-reading-image"><img src="${esc(source.imageUrl)}" alt="這次判讀對照的圖卡"></div>${source.commentA?`<div class="ai-reading-comments"><section><h3>甲</h3><p>${esc(source.commentA)}</p></section><section><h3>乙</h3><p>${esc(source.commentB)}</p></section></div>`:''}</section>`;
