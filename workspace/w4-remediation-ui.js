import {hasHumanFeedback,w4Progress} from './w4-progress.js';
export function createW4RemediationUI({state,api,show,esc,images,board,say,dialog}){
 const human=hasHumanFeedback;
 const status=w=>w4Progress(w).status;
 ...
 function workTools(w,teacher){if(state.board.activity.kind!=='w4')return '';return `<p><span class="tag">${status(w)}</span></p><p class="muted">${esc(w4Progress(w).detail)}</p>${feedback(w)}${teacher&&w.versions.length&&!state.board.activity.archived&&state.board.activity.accepting&&['review','exhibit'].includes(state.board.activity.phase)?`<button class="secondary" data-action="teacherReading" data-id="${esc(w.id)}">由老師補上初讀</button>`:''}${!teacher&&human(w)&&!w.decisions.length?'<p class="notice">請自己對照回饋、原本想表達的意思與畫面線索，決定修改或保留，並說明理由。保留請按「有理由保留目前版本」；修改請上傳 V2，填寫修改依據。不使用 AI 代答。</p>':''}`;}
 function reminders(link=true){return (state.home.reminders||[]).map(r=>`<section class="panel w4-reminder-warning" aria-label="W4 補交提醒"><h2><span aria-hidden="true">⚠️</span> W4 作品待補交</h2><p>${esc(r.message)}</p>${r.deadline?`<p><strong>補交日期：${esc(r.deadline)}</strong></p>`:''}${link?`<button data-action="activity" data-id="${esc(r.activityId)}">補交 W4 作品</button>`:'<p>閱讀並確認下方登入說明後，即可進入作品區補交。</p>'}</section>`).join('');}
