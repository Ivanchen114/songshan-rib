...
async function galleryRequest(action,data){const view={galleryPage:'gallery',galleryPreview:'preview',galleryCheck:'check'}[action],q=new URLSearchParams({view,...data,...(data.ids?{ids:data.ids.join(',')}:{})});const res=await fetch('/api/portfolio?'+q,{cache:'no-store',signal:AbortSignal.timeout(15000)}),r=await res.json();if(!res.ok||!r.ok)throw Error('展覽暫時無法載入，請稍後重試。');return r.data;}
async function load(){if(loading)return;loading=true;$('#reloadCatalog').disabled=true;
 try{config||=await(await fetch('config.json')).json();const requested=Number(new URLSearchParams(location.search).get('week'));if(config.courses.some(c=>c.week===requested)){location.replace(entry({week:requested}));return;}render({courses:[],collections:[]});gallery?.destroy();gallery=createArtworkGallery({root:$('#artworkGallery'),request:galleryRequest,open:item=>{window.open(entry({exhibit:item.id}),'_blank','noopener');}});const response=await fetch('/api/portfolio',{cache:'no-store',signal:AbortSignal.timeout(15000)}),r=await response.json();if(!response.ok||!r.ok||!Array.isArray(r.data?.collections)||!Array.isArray(r.data?.courses))throw Error('unavailable');render(r.data);}
