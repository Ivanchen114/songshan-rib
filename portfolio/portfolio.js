...
const entry=args=>config.gasUrl+'#'+new URLSearchParams(args);const badge=w=>w===15?'W15–W16':w?'W'+w:'作品';
function render(d){
 ...
 $('#courseEntries').innerHTML=rooms.map(c=>{const args={week:c.week};if(c.legacyCode)args.code=c.legacyCode;return `<a class="course-entry" data-course="w${c.week}" href="${esc(entry(args))}" target="_blank" rel="noopener noreferrer"><span class="week-label">${badge(c.week)}</span><span class="entry-copy"><strong>${esc(c.title.replace(/^W\d+(?:–W\d+)?\s*/,''))}</strong><small>${c.accepting===false?'尚未開放上傳':esc(courseNotes[c.week]||(c.week===15?'五頁作品・跨週接續':'課堂作品與歷程'))}</small></span><span class="entry-arrow" aria-hidden="true">↗</span><span class="sr-only">（另開分頁）</span></a>`;}).join('')+`<a class="course-entry" data-course="w18" href="${esc(entry({week:18}))}" target="_blank" rel="noopener noreferrer"><span class="week-label">W18</span><span class="entry-copy"><strong>自願選期末精選</strong><small>選既有作品・不用重新上傳</small></span><span class="entry-arrow">↗</span></a>`;
