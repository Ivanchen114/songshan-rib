...
module.exports=async function handler(req,res){
 ...
 try{
  ...
  const courses=d.courses.filter(x=>/^[A-F0-9]{6}$/.test(x.code)).map(x=>({code:x.code,title:str(x.title,60),term:str(x.term,40),week:week(x.week),accepting:x.accepting===true}));
