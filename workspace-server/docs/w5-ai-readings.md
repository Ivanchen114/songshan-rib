...
Teacher authorization on 2026-09-24 adds a student-visible record under `teacher_notes.studentRecord`. Only records with `published:true` expose allowlisted record kind and six narrative fields to the owner. These include actual AI/teacher exchanges, uncertainty and interpretation basis; never imply an individual teacher reply when none was recorded. Raw internal notes and peer access remain restricted.

Attribution follows actual participation: `kind:ai` has no teacher-reply text and uses an AI-only heading; `kind:joint` summarizes an actual recorded teacher exchange, preserving uncertainty. General approval is never represented as individual teacher authorship.
