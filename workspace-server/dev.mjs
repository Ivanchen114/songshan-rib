...
http.createServer(async(req,res)=>{try{
  ...
  if(url.pathname==='/__demo/student'&&process.env.RIB_DEMO_GROUP==='true'&&req.method==='GET'){const p=f.people[Number(url.searchParams.get('seat'))-1];if(!p){res.writeHead(404);return res.end();}const session=await createSession(f.db,{role:'student',term:p.term,studentId:p.studentId,revision:1});res.setHeader('Set-Cookie',`rib_session=${session.token}; HttpOnly; SameSite=Lax; Path=/`);res.writeHead(302,{Location:'/workspace/?week='+(process.env.RIB_DEMO_W8_MATERIALS==='true'?'8':process.env.RIB_DEMO_W7_NEWS==='true'?'7':'5')});return res.end();}
