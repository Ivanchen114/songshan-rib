...
export const TABLES=['students','credentials','teachers','activities','works','members','versions','reviews','replies','decisions','uploads','assessments','publications','events','legacy_records','legacy_files','migration_runs','workspace_state','term_drafts','term_changes'];
...
export async function snapshot(db){return db.transaction(async tx=>{
 ...
 const payload={format:'rib-backup-v2',created:new Date().toISOString(),tables};return {...payload,sha256:sha(JSON.stringify(payload))};
});}
export async function restoreEmpty(db,backup){
 const {sha256,...payload}=backup;demand(['rib-backup-v1','rib-backup-v2'].includes(payload.format)&&sha(JSON.stringify(payload))===sha256,409,'備份雜湊不一致。');
