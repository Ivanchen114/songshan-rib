...
export class TopicSelection extends GroupMembership{
 ...
 async drawTopic(p,input){
  ...
  return this.db.transaction(async db=>{
   ...
   demand(w.kind==='w7-news'&&p.student.is_test===w.test_only,403,'請在自己的新聞與研究小組抽題。');
   if(w7Topic(w.topic))return {saved:true,workId:w.id,topic:w.topic};
   ...
   const prior=await db.query(`select old.id from rib.works old join rib.members om on om.work_id=old.id
    ...
    and current.work_id=$2 and current.status='confirmed' limit 1`,[w.activity_id,w.id,W7_TOPICS.map(t=>t.id)]);
   ...
   const counts=await db.query(`select topic,count(*)::int as total,count(*) filter(where class_name=$2)::int as class_total
    from rib.works where activity_id=$1 and topic=any($3::text[]) group by topic`,[w.activity_id,w.class_name,W7_TOPICS.map(t=>t.id)]);
   const all=W7_TOPICS.map(t=>({id:t.id,...counts.find(c=>c.topic===t.id)}));
