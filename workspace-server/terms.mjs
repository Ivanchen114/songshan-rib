...
export async function guardActivityWrite(db,action,input){
 ...
 if(['drawTopic','testFeedback','assignReader','invite','leaveGroup','prepare','decision','consent','assess','wallComment','wallVote','markCurrent','paperKeep'].includes(action)&&input.workId)activityId=(await db.query('select activity_id from rib.works where id=$1',[input.workId]))[0]?.activity_id;
