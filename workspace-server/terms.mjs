...
export const activityWrites=['revealAuthors','saveReflection','reviewReflection','drawTopic','saveAiJudgment','chooseTopic','testFeedback','assignReader','reply','ensureWork','invite','leaveGroup','invitation','prepare','finalize','dispatch','review','requestReplacement','replace','decision','control','consent','publish','assess','wallComment','wallVote','moderateComment','markCurrent','paperKeep','selectionSave','selectionFeature','referencePrepare','referenceFinalize'];
export async function guardActivityWrite(db,action,input){
 let activityId=['revealAuthors','saveAiJudgment','chooseTopic','ensureWork','invitation','dispatch','control','referencePrepare'].includes(action)?input.activityId:null;
 if(['saveReflection','reviewReflection','drawTopic','testFeedback','assignReader','invite','leaveGroup','prepare','decision','consent','assess','wallComment','wallVote','markCurrent','paperKeep'].includes(action)&&input.workId)activityId=(await db.query('select activity_id from rib.works where id=$1',[input.workId]))[0]?.activity_id;
