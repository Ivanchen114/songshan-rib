...
export async function importSnapshot(db,store,manifest,files) {
  ...
  return db.transaction(async tx=>{
    ...
    for(const c of tables.Consents||[]){const work=(tables.Teams||[]).find(t=>(members.get(t.id)?.confirmed||[]).some(sid=>c.id===t.id+':'+sid));if(!work)continue;const sid=members.get(work.id).confirmed.find(sid=>c.id===work.id+':'+sid);await tx.query('update rib.members set consent=$1,sharing_opt_out=$2 where work_id=$3 and student_id=$4',[c.allowed===true,c.allowed===false,work.id,sid]);}
