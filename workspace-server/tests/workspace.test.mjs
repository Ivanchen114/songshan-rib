...
test('W5 adds members directly and prevents duplicate groups',async t=>{const f=await fixture();t.after(f.close);const s=new Workspace(f.db,f.store),[a,b]=f.people,w=await s.ensureWork(a,{activityId:'w5-demo'});await s.invite(a,{workId:w.id,studentIds:[b.studentId]});await s.work(b,w.id,{write:true});assert.equal((await s.ensureWork(b,{activityId:'w5-demo'})).id,w.id);await assert.rejects(s.invite(a,{workId:w.id,studentIds:[b.studentId]}),/小組/);});
